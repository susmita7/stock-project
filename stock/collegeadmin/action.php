...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['read'])) {
  ...
  if (mysqli_num_rows($fire)>0) {
      ...
      if (!isset($datas['clg_admin_img_link']) || !file_exists($datas['clg_admin_img_link'])) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readcount'])) {
  ...
  $query = "SELECT * FROM clg_admin_notify WHERE status=0 AND clg_id='".$_SESSION['clg_id']."'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readnotify'])) {
  ...
  $query_new = "SELECT * FROM clg_admin_notify WHERE status=0 AND clg_id='".$_SESSION['clg_id']."'";
  ...
  $query = "SELECT * FROM clg_admin_notify WHERE clg_id='".$_SESSION['clg_id']."' ORDER BY notify_id DESC LIMIT 3";
        ...
        if ($notify['notify_from']==0) {
          ...
          $records .='<div onclick="markRead('.$notify['notify_id'].')" class="notification">
                        <div class="alert_icon">  
                          ...
                          <div class="noti_content">
                            <p><b>From '.$noti['dept_name'].' Department</b></p>
                            <p>'.$notify['notify_message'].'</p>
                            <div class="alert-time">'.$time.'</div>
                          </div>
                      </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readall'])) {
  ...
  if (mysqli_num_rows($fire)>0) {
    ...
    $query ="UPDATE clg_admin_notify SET status=1 WHERE status=0 AND clg_id='".$_SESSION['clg_id']."'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['markread'])) {
  ...
  if (isset($_POST['id'])) {
    ...
    $query = "SELECT * FROM clg_admin_notify WHERE notify_id='$id' AND clg_id='".$_SESSION['clg_id']."'";
    $fire  = mysqli_query($con,$query) or die("can not show data from database".mysqli_error($con));
    ...
    if (mysqli_num_rows($fire)>0) {
      ...
      if ($row['status']==0) {
        ...
        $query ="UPDATE clg_admin_notify SET status=1 WHERE notify_id='$id' AND clg_id='".$_SESSION['clg_id']."'";
