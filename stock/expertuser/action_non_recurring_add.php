...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readstock'])) {
  ...
  $query = "SELECT * FROM non_recurring_stock WHERE dept_id='".$_SESSION['dept_id']."' ORDER BY stock_id DESC";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['adds1'])) {
  ...
  if ($_POST['datap']!=null) {
    ...
    $fire = mysqli_query($con,$query) or die("can not access".mysqli_error($con));
      ?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
  ...
  //check if the (item+unit+dept) is new to backend table
  ...
  if (mysqli_num_rows($fire) == 0) {
    ...
    $query = "INSERT INTO `non_recurring_stock_levels`(`name`, `unit`, `office_quantity`, `lab_quantity`, `updated`, `dept_id`) VALUES ('$particular_name','$unit','$office_quantity','$lab_quantity','$updated','$dept_id')";
    ...
    if ($f) {
      ...
      $activity_body= "You have added stock into non-recurring stock register : ".$quantity." ".$unit." of ".$particular_name." (".$used_for.").";
    ...
    //$query = "UPDATE non_recurring_stock_levels SET quantity='$quantity1',updated='$updated' WHERE used_for='$used_for' AND name='$particular_name' AND unit='$unit' AND dept_id='$dept_id'";

    $query = "UPDATE `non_recurring_stock_levels` SET `office_quantity`='$quantity1',`lab_quantity`='$quantity2',`updated`='$updated' WHERE id='".$item['id']."'";
