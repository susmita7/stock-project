...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['read'])) {
  ...
  if (mysqli_num_rows($fire)>0) {
      ...
      if (!isset($datas['dept_admin_img_link']) || !file_exists($datas['dept_admin_img_link'])) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readcount'])) {
  ...
  $query = "SELECT * FROM dept_admin_notify WHERE status=0 AND dept_id='".$_SESSION['dept_id']."'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readnotify'])) {
  ...
  $query_new = "SELECT * FROM dept_admin_notify WHERE status=0 AND dept_id='".$_SESSION['dept_id']."'";
  ...
  $query = "SELECT * FROM dept_admin_notify WHERE dept_id='".$_SESSION['dept_id']."' ORDER BY notify_id DESC LIMIT 3";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readall'])) {
  ...
  if (mysqli_num_rows($fire)>0) {
    ...
    $query ="UPDATE dept_admin_notify SET status=1 WHERE status=0 AND dept_id='".$_SESSION['dept_id']."'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['markread'])) {
  ...
  if (isset($_POST['id'])) {
    ...
    $query = "SELECT * FROM dept_admin_notify WHERE notify_id='$id' AND dept_id='".$_SESSION['dept_id']."'";
    $fire  = mysqli_query($con,$query) or die("can not show data from database".mysqli_error($con));
    ...
    if (mysqli_num_rows($fire)>0) {
      ...
      if ($row['status']==0) {
        ...
        $query ="UPDATE dept_admin_notify SET status=1 WHERE notify_id='$id' AND dept_id='".$_SESSION['dept_id']."'";
