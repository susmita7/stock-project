...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['readactivities'])) {
  ...
  $query = "SELECT * FROM dept_admin_activity WHERE dept_admin_id='".$_SESSION['dept_admin_id']."' ORDER BY activity_id DESC";
  ...
  if (mysqli_num_rows($fire)>0) {
    ...
    while ($activity = mysqli_fetch_assoc($fire)) {
      ...
      }elseif ($activity['activity_type']=="expertuser") {
        $records .='<i class="fas fa-user-lock"></i>';
      }elseif ($activity['activity_type']=="orders") {
        $records .='<i class="fa fa-file-pdf-o"></i>';
      }elseif ($activity['activity_type']=="stockgroup") {
        $records .='<i class="fa fa-object-group"></i>';
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['clear'])){
  ...
  if (!empty($_POST['delid'])) {
    ...
    $query ="DELETE FROM dept_admin_activity WHERE activity_id='$delid'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['clearall'])) {
  ...
  $query = "SELECT * FROM dept_admin_activity WHERE dept_admin_id='$dept_admin_id'";
  $fire  = mysqli_query($con,$query) or die("can not show data from database".mysqli_error($con));

  if (mysqli_num_rows($fire)>0) {

    $query ="DELETE FROM dept_admin_activity WHERE dept_admin_id='$dept_admin_id'";
