...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['sendotp'])) {
  ...
  if (!empty($_POST['dept_admin_email'])) {
    ...
    if ($fire) {
      ...
      if (mysqli_num_rows($fire) == 1) {
        ...
        $full_name=$row['dept_admin_first_name']." ".$row['dept_admin_last_name'];
        ...
        $mail_status = send_email($dept_admin_email,$full_name,$otp);
        ...
        if ($mail_status==1) {
          ...
          $query = "UPDATE `department_admin` SET `created_at`='$created_at',`otp`='$otp',`is_expired`=0 WHERE dept_admin_id='$dept_admin_id'";
          ...
          if (!empty($dept_admin_id)) {
            //$msg = array("icon"=>"success", "currentid"=>"".$current_id, "value"=>"".$otp);
            $msg = array("icon"=>"success", "currentid"=>"".$dept_admin_id, "text"=>"otp successfully sent");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['otp'])) {
  ...
  if (!empty($_POST['otp_id']) && !empty($_POST['otp_val'])) {
    ...
    $query = "SELECT * FROM `department_admin` WHERE dept_admin_id ='$otp_id' AND is_expired !=1 AND NOW() <= DATE_ADD(created_at, INTERVAL 10 MINUTE)";
    $fire  = mysqli_query($con,$query) or die("can not show data from database. ".mysqli_error($con));
    $row   = mysqli_fetch_assoc($fire);
    ...
    if ($fire) {
      ...
      if (mysqli_num_rows($fire) == 1) {
        ...
        if($row['otp']==$otp_val && $row['is_expired']==0) {
          ...
          $query = "UPDATE `department_admin` SET is_expired= 1 WHERE dept_admin_id='$otp_id'";

          $fire = mysqli_query($con,$query) or die("can not update data. ".mysqli_error($con));

          if ($fire) {
            $msg = array("icon"=>"success", "id"=>"".$dept_admin_id, "text"=>"OTP successfully matched!");    
            echo json_encode($msg);
          }else{
            $msg = array("icon"=>"error", "title"=>"Oops", "text"=>"Something went wrong!");    
            echo json_encode($msg);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['resetpassword'])) {
  ...
  if (!empty($_POST['dept_admin_id']) && !empty($_POST['dept_admin_password'])) {
    ...
    if ($fire) {
      ...
      if (mysqli_num_rows($fire) == 1) {
        ...
        if(strlen($dept_admin_password)>=4) {
                      
          $dept_admin_password=md5($dept_admin_password);
                      
          $query = "UPDATE department_admin SET dept_admin_password='$dept_admin_password' WHERE dept_admin_id='$dept_admin_id'";
